			...
			try:
				...
				if recv_msg.type == 'end':
					...
					for h in handle_list:
						...
						self.logger.info('창 [' + str(window_title) + '] 위치 정보 현재 위치로 업데이트: ' + str((anchor_x, anchor_y)))
					...
					if cmd == 'show':
						for each_hwnd in handle_list:
							if each_hwnd in parent_hwnds:
								self.win.setVisible(parent_hwnds[each_hwnd])
							else:
								self.win.setVisible(each_hwnd)
					else:
						for each_hwnd in handle_list:
							if each_hwnd in parent_hwnds:
								self.win.setInvisible(parent_hwnds[each_hwnd])
							else:
