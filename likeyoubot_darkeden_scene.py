...
class LYBDarkEdenScene(likeyoubot_scene.LYBScene):
    ...
    def quick_move_gold_scene(self):

        if self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_ETC + 'quick_move_gold') == True:
            self.lyb_mouse_click(self.scene_name + '_ok', custom_threshold=0)
        else:
            self.lyb_mouse_click(self.scene_name + '_close_icon', custom_threshold=0)
    ...
    def upjeok_scene(self):
        ...
        if self.status == 0:
            ...
            self.status = self.get_option('last_status')
        elif self.status == self.get_work_status('일일임무'):
            self.status = 200
        elif self.status == 200:
            self.lyb_mouse_click('upjeok_scene_tab_1', custom_threshold=0)
            self.status += 1    
        elif 201 <= self.status < 210:
            self.status += 1
            if self.click_resource('upjeok_scene_ilil_bosang_loc') is True:
                return self.status  

            self.status = 210
        elif 210 <= self.status < 213:
            pb_name = 'upjeok_scene_ilil_complete_bosang_' + str(self.status - 210)
            self.lyb_mouse_click(pb_name, custom_threshold=0)
            self.status += 1
        else:
            if self.scene_name + '_close_icon' in self.game_object.resource_manager.pixel_box_dic:
    ...
    def gejo_scene(self):
        ...
        if self.status == 0:
            ...
            for i in range(len(lybgamedarkeden.LYBDarkEden.option_list)):
                ...
                resource_name = 'gejo_scene_bunhe_option_' + str(i) + '_loc'
                ...
                if loc_x != -1 and cfg_item is False:
                    is_clicked = True
                    self.lyb_mouse_click(pb_name, custom_threshold=0)
                elif loc_x == -1 and cfg_item is True:
                    is_clicked = True
                    self.lyb_mouse_click(pb_name, custom_threshold=0)
            ...
            for i in range(len(lybgamedarkeden.LYBDarkEden.item_quality_list)):
                ...
                resource_name = 'gejo_scene_item_quality_' + str(i) + '_loc'
                (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart2(
                    self.window_image,
                    resource_name,
                    near=16,
                    custom_threshold=0.5,
                    custom_below_level=quality_list[i][0],
                    custom_top_level=quality_list[i][1],
                    custom_flag=1,
                    debug=True)
                self.logger.debug(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
                if loc_x != -1 and cfg_item_quality is False:
                    is_clicked = True
                    self.lyb_mouse_click(pb_name, custom_threshold=0)
                elif loc_x == -1 and cfg_item_quality is True:
                    is_clicked = True
                    self.lyb_mouse_click(pb_name, custom_threshold=0)
            ...
            #     # self.logger.debug(pb_name + ' ' + str(match_rate))
            #     if match_rate > 0.7 and cfg_item is False:
            #         is_clicked = True
            #         self.lyb_mouse_click(pb_name, custom_threshold=0)
            #     elif match_rate < 0.7 and cfg_item is True:
            #         is_clicked = True
            #         self.lyb_mouse_click(pb_name, custom_threshold=0)
            ...
            #     match_rate = self.game_object.rateMatchedPixelBox(self.window_pixels, pb_name, custom_tolerance=10)
            #     # self.logger.debug(pb_name + ' ' + str(match_rate))
            #     if match_rate > 0.7 and cfg_item_quality is False:
            #         is_clicked = True
            #         self.lyb_mouse_click(pb_name, custom_threshold=0)
            #     elif match_rate < 0.7 and cfg_item_quality is True:
            #         is_clicked = True
            #         self.lyb_mouse_click(pb_name, custom_threshold=0)
    ...
    def special_dungeon_scene(self):
        ...
        resource_name = 'special_dungeon_create_scene_create_loc'
        (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart2(
            self.window_image,
            resource_name,
            custom_threshold=0.7,
        ...
        self.logger.debug(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
        if loc_x != -1:
            return self.game_object.get_scene('special_dungeon_create_scene').process(self.window_image, self.window_pixels)
    ...
    def gabang_sell_scene(self):
        ...
        if self.status == 0:
            ...
            for i in range(len(lybgamedarkeden.LYBDarkEden.item_list)):
                ...
                resource_name = 'gabang_sell_scene_item_' + str(i) + '_loc'
                (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart2(
                    self.window_image,
                    resource_name,
                    near=8,
            ...
            for i in range(len(lybgamedarkeden.LYBDarkEden.item_quality_list)):
                ...
                resource_name = 'gabang_sell_scene_item_quality_' + str(i) + '_loc'
                (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart2(
                    self.window_image,
                    resource_name,
                    near=16,
    ...
    def jido_scene(self):
        ...
        if self.status == 0:
            ...
            self.game_object.get_scene('main_scene').set_checkpoint('auto_move_check_period')
            ...
            for i in range(6):
                ...
                if cfg_dungeon_floor != '선택안함':
                    ...
                    isMatched, match_rate = self.click_resource2(resource_name, custom_threshold=0.7) 
                    if isMatched is True:
                        e = {}
                        e['index'] = i
                        e['rate'] = match_rate
                        if p is None or match_rate < e['rate']:
                            p = e
            if p is not None:
                i = p['index']
                cfg_dungeon_floor = self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_ETC + 'dungeon_floor' + str(i))
            ...
            if match_rate > 0.8:
                ...
                for i in range(6):                    
                    (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart(
                        ...
                        custom_threshold=0.7,
                        custom_flag=1,
                        custom_rect=(570, 180 + (i*40) - 25, 720, 180 + (i*40) + 55),
                        debug=True
                        )
                    self.logger.debug(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate) + str((570, 180 + (i*40) - 25, 720, 180 + (i*40) + 55)))
                    if loc_x != -1:
                        # p['rate'] = match_rate
                        # p['loc'] = (loc_x, loc_y)
                ...
                if len(sorted_list) > cfg_order - 1:
                    loc_x, loc_y = sorted_list[cfg_order - 1][0]
                    self.lyb_mouse_click_location(loc_x, loc_y)
                    self.status = 10
                    return self.status
    ...
    def buy_confirm_scene(self):
        ...
        if self.status == 0:
            ...
            self.click_resource('buy_confirm_scene_gume_loc')
            self.status = 99999
        else:
            if self.scene_name + '_close_icon' in self.game_object.resource_manager.pixel_box_dic:
                self.lyb_mouse_click(self.scene_name + '_close_icon', custom_threshold=0)
    ...
    def sangjeom_scene(self):
        ...
        resource_name = 'buy_confirm_scene_gume_loc'
        (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart2(
            self.window_image,
            resource_name,
            custom_threshold=0.7,
        ...
        self.logger.debug(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
        if loc_x != -1:
            return self.game_object.get_scene('buy_confirm_scene').process(self.window_image, self.window_pixels)
        ...
        if self.status == 0:
            ...
            self.status += 1
        elif self.status == self.get_work_status('뽑기'):
            self.status = 100
        elif self.status == 100:
    ...
    def death_scene(self):
        ...
        if self.status == 0:
            ...
            self.set_option('click_help', False)
            self.set_option('click_help_guild', False)
            self.status += 1
        elif 1 <= self.status < 10:
            self.status += 1
            resource_name = 'death_scene_help_loc'
            if self.click_resource(resource_name) is True and self.get_option('click_help') is False:
                self.set_option('click_help', True)
                return self.status

            resource_name = 'death_scene_help_guild_loc'
            if self.click_resource(resource_name) is True and self.get_option('click_help_guild') is False:
                self.set_option('click_help_guild', True)
                return self.status
    ...
    def quest_scene(self):
        ...
        if self.status == 0:
            ...
            self.set_option('jiyeok_quest_drag_count', 0)
            self.status = 100
        elif 100 <= self.status < 105:
            self.status += 1
            resource_name = 'quest_scene_jiyeok_count_loc'
            (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart(
                self.window_image,
                resource_name,
                custom_threshold=0.7,
            ...
            self.status += 1
            cfg_area = self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_WORK + 'jiyeok_quest_area')
            resource_name = 'quest_scene_jiyeok_' + cfg_area + '_loc'
            if self.click_resource(resource_name) is True:
                self.status = 120
                return self.status
        elif 120 <= self.status < 125:
            self.status += 1
            cfg_quest = self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_WORK + 'jiyeok_quest_select')
            resource_name = 'quest_scene_jiyeok_quest_' + cfg_quest + '_loc'
            for i in range(6):
                (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart(
                    self.window_image,
                    resource_name,
            ...
            count = self.get_option('jiyeok_quest_drag_count')
            if count is None:
                count = 0
            ...
            if count > 3:
                ...
                self.set_option('jiyeok_quest_drag_count', count + 1)
                self.lyb_mouse_drag('quest_scene_jiyeok_drag_bot', 'quest_scene_jiyeok_drag_top')
                self.status = 120
        else:
            if self.scene_name + '_close_icon' in self.game_object.resource_manager.pixel_box_dic:
                self.lyb_mouse_click(self.scene_name + '_close_icon', custom_threshold=0)
    ...
    def main_scene(self):
        ...
        if self.status == 0:
            ...
            cfg_duration = int(self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_WORK + 'main_quest_duration'))
            elapsed_time = self.get_elapsed_time()
            if cfg_duration != 0 and elapsed_time > self.period_bot(cfg_duration):
            ...
            elapsed_time = self.get_elapsed_time()
            if cfg_duration != 0 and elapsed_time > cfg_duration:
                self.set_option(self.current_work + '_end_flag', True)
            ...
            if cfg_check_ilil_quest != 0 and elapsed_time > cfg_check_ilil_quest:
        ...
        elif (  self.status == self.get_work_status('종족임무') or
                self.status == self.get_work_status('일일임무') ):
            ...
            self.game_object.get_scene('sangjeom_scene').status = self.status
    ...
    def pre_process_main_scene(self):
        ...
        if elapsed_time > self.period_bot(3) and self.game_object.get_scene('jido_scene').get_option('click_jido') is False:
            (loc_x, loc_y), match_rate = self.game_object.locationResourceOnWindowPart(
                self.window_image,
                resource_name,
                custom_threshold=0.6,
        ...
        if elapsed_time > self.period_bot(10) and self.game_object.get_scene('jido_scene').get_option('click_jido') is True:
            ...
            if loc_x != -1:
                ...
                if limit_count >= 0:
                    ...
                    self.game_object.get_scene('jido_scene').set_option('click_jido', False)
                    cfg_check_worldmap = int(self.get_game_config(lybconstant.LYB_DO_STRING_DARKEDEN_WORK + 'auto_move_check_period'))
                    self.game_object.get_scene('main_scene').checkpoint['auto_move_check_period'] = time.time() - cfg_check_worldmap + 5
                    self.game_object.get_scene('main_scene').checkpoint['main_scene_quick_move'] = 0
    ...
    def is_complete_quest(self, limit=3):
        complete_list = [
            ...
            'main_scene_quest_jongjok_complete_loc',
        ]
