...
import likeyoubot_coc_scene as lybscene
...
class LYBCOC(lybgame.LYBGame):
	...
	def __init__(self, game_name, game_data_name, window):
		lybgame.LYBGame.__init__(self, lybconstant.LYB_GAME_COC, lybconstant.LYB_GAME_DATA_COC, window)

	def process(self, window_image):
		rc = super(LYBCOC, self).process(window_image)
		if rc < 0:
			return rc
	...
	def get_screen_by_location(self, window_image):
		...
		scene_name = self.army_scene(window_image)
		if len(scene_name) > 0:
			return scene_name
	...
	def army_scene(self, window_image):
		(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
							self.window_image,
							'army_scene',
							custom_threshold=0.7,
							custom_flag=1,
							custom_rect=(580, 30, 620, 65)
							)
		self.logger.debug('army_scene:' + str(match_rate))
		if loc_x != -1:
			return 'army_scene'
	...
	def scene_init_screen(self, window_image):
		...
		if self.player_type == 'nox':
			for each_icon in LYBCOC.coc_icon_list:
	...
	def add_scene(self, scene_name):
		self.scene_dic[scene_name] = lybscene.LYBCOCScene(scene_name)
		self.scene_dic[scene_name].setLoggingQueue(self.logging_queue)
		self.scene_dic[scene_name].setGameObject(self)

class LYBCOCTab(lybgame.LYBGameTab):
	def __init__(self, root_frame, configure, game_options, inner_frame_dics, width, height, game_name=lybconstant.LYB_GAME_COC):
	...
	def set_work_list(self):
		...
		for each_work in LYBCOC.work_list:
