...
import likeyoubot_sajo as lybgamesajo
from likeyoubot_configure import LYBConstant as lybconstant
import likeyoubot_scene
import time

class LYBSajoScene(likeyoubot_scene.LYBScene):
	def __init__(self, scene_name):
		likeyoubot_scene.LYBScene.__init__(self, scene_name)

	def process(self, window_image, window_pixels):

		super(LYBSajoScene, self).process(window_image, window_pixels)
		...
		if self.scene_name == 'init_screen_scene':
			...
			rc = self.main_scene()
		elif self.scene_name == 'daily_reward_scene':
			rc = self.daily_reward_scene()
	...
	def daily_reward_scene(self):
		...
		if self.status == 0:
			...
			self.status += 1
			pb_name = 'daily_reward_scene_chulseok'
	...
	def init_screen_scene(self):
		...
		if self.game_object.player_type == 'nox':
			for each_icon in lybgamesajo.LYBSajo.sajo_icon_list:
	...
	def main_scene(self):
		...
		if self.status == 0:
			...
			elif inner_status >= 1 and inner_status < 60:
				resource_name_list = [
					'main_scene_quest_complete_loc',
					'main_scene_quest_reward_loc'
					]
	...
	def pre_process_main_scene(self):
		...
		match_rate = self.game_object.rateMatchedPixelBox(self.window_pixels, pb_name)
		if match_rate > 0.9:
			self.logger.debug(pb_name + ' ' + str(match_rate) + ' ' + '던전')						
			return self.status
	...
	def get_work_status(self, work_name):
		if work_name in lybgamesajo.LYBSajo.work_list:
			return (lybgamesajo.LYBSajo.work_list.index(work_name) + 1) * 1000
		else: 
			return 99999