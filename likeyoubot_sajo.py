...
import likeyoubot_sajo_scene as lybscene
...
class LYBSajo(lybgame.LYBGame):
	...
	def __init__(self, game_name, game_data_name, window):
		lybgame.LYBGame.__init__(self, lybconstant.LYB_GAME_SAJO, lybconstant.LYB_GAME_DATA_SAJO, window)

	def process(self, window_image):
		rc = super(LYBSajo, self).process(window_image)
		if rc < 0:
			return rc
	...
	def custom_check(self, window_image, window_pixel):
		...
		resource_name = 'npc_close_loc'
		(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
							self.window_image,
							resource_name,
							custom_threshold=0.9,
							custom_flag=1,
							custom_rect=(540, 270, 580, 300)
							)
		if loc_x != -1:
			self.logger.debug('닫기: ' + str(match_rate))
			self.get_scene('main_scene').lyb_mouse_click_location(loc_x, loc_y - 30)
		...
		(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
							...
							custom_threshold=0.9,
							custom_flag=1,
							custom_rect=(550, 35, 605, 55)
							)
		if loc_x != -1:
			self.logger.debug('건너뛰기: ' + str(match_rate))
	...
	def scene_init_screen(self, window_image):
		...
		if self.player_type == 'nox':
			for each_icon in LYBSajo.sajo_icon_list:
	...
	def add_scene(self, scene_name):
		self.scene_dic[scene_name] = lybscene.LYBSajoScene(scene_name)
		self.scene_dic[scene_name].setLoggingQueue(self.logging_queue)
		self.scene_dic[scene_name].setGameObject(self)

class LYBSajoTab(lybgame.LYBGameTab):
	def __init__(self, root_frame, configure, game_options, inner_frame_dics, width, height, game_name=lybconstant.LYB_GAME_SAJO):
	...
	def set_work_list(self):
		...
		for each_work in LYBSajo.work_list:
