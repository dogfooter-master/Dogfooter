...
import likeyoubot_linm_scene as lybscene
...
class LYBLinm(lybgame.LYBGame):
	...
	def __init__(self, game_name, game_data_name, window):
		lybgame.LYBGame.__init__(self, lybconstant.LYB_GAME_LINM, lybconstant.LYB_GAME_DATA_LINM, window)

	def process(self, window_image):
		rc = super(LYBLinm, self).process(window_image)
		if rc < 0:
			return rc
	...
	def custom_check(self, window_image, window_pixel):
		if self.main_scene != None and self.main_scene.current_work == '메인 퀘스트':		
			resource_name_list = [
				'tutorial_map_move_loc',
			]	
			for resource_name in resource_name_list:
				(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
									...
									custom_flag=1,
									custom_rect=(200, 70, 440, 120)
									)
				if loc_x != -1:
					self.logger.info(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
					self.window.mouse_click(self.hwnd, loc_x, loc_y)
					return resource_name

		resource_name = 'npc_talk_skip_loc'
		(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
							self.window_image,
							resource_name,
							custom_threshold=0.7,
							custom_flag=1,
							custom_rect=(540, 250, 610, 290)
							)
		if loc_x != -1:
			self.logger.info(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
			self.window.mouse_click(self.hwnd, loc_x, loc_y)
		...
		resource_name = 'reward_loc'
		(loc_x, loc_y), match_rate = self.locationResourceOnWindowPart(
							self.window_image,
							resource_name,
							custom_threshold=0.7,
							custom_flag=1,
							custom_rect=(310, 140, 350, 180)
							)
		if loc_x != -1:
			self.logger.info(resource_name + ' ' + str((loc_x, loc_y)) + ' ' + str(match_rate))
			self.window.mouse_click(self.hwnd, loc_x, loc_y)
	...
	def scene_init_screen(self, window_image):
		...
		if self.player_type == 'nox':
			for each_icon in LYBLinm.linm_icon_list:
	...
	def add_scene(self, scene_name):
		self.scene_dic[scene_name] = lybscene.LYBLinmScene(scene_name)
		self.scene_dic[scene_name].setLoggingQueue(self.logging_queue)
		self.scene_dic[scene_name].setGameObject(self)

class LYBLinmTab(lybgame.LYBGameTab):
	def __init__(self, root_frame, configure, game_options, inner_frame_dics, width, height, game_name=lybconstant.LYB_GAME_LINM):
	...
	def set_work_list(self):
		...
		for each_work in LYBLinm.work_list:
	...
	def set_option(self):
		...
		self.option_dic[lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death'] = tkinter.BooleanVar(frame)
		self.option_dic[lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death'].trace(
			'w', lambda *args: self.callback_notify_character_death(args, lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death')
			)
		if not lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death' in self.configure.common_config[self.game_name]:
			self.configure.common_config[self.game_name][lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death'] = True

		check_box = ttk.Checkbutton(

			master 				= frame,
			text 				= '캐릭터 사망', 
			variable 			= self.option_dic[lybconstant.LYB_DO_STRING_LINM_NOTIFY + 'character_death'],
			onvalue 			= True, 
			offvalue 			= False
		)
