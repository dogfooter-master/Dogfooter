...
class LYBLoginGUI:
	...
	def __init__(self, master, configure):
		...
		if self.configure.common_config[lybconstant.LYB_DO_BOOLEAN_SAVE_LOGIN_ACCOUNT] != True:
		...
		if not lybconstant.LYB_DO_BOOLEAN_SAVE_LOGIN_ACCOUNT + '_passwd' in self.configure.common_config:
			...
			self.option_dic[lybconstant.LYB_DO_BOOLEAN_SAVE_LOGIN_ACCOUNT + '_passwd'].set(
				self.lyblicense.get_decrypt(self.configure.common_config[lybconstant.LYB_DO_BOOLEAN_SAVE_LOGIN_ACCOUNT + '_passwd'])
				)
		...
		self.option_dic[lybconstant.LYB_DO_BOOLEAN_AUTO_UPDATE].set(self.configure.common_config[lybconstant.LYB_DO_BOOLEAN_AUTO_UPDATE])
...
class LYBUpdateWorker(threading.Thread):
	...
	def run(self):
		...
		# content_length = header.get('content-length', None)
			...
			try:
				shutil.move(likeyoubot_configure.LYBConfigure.resource_path(file_name), likeyoubot_configure.LYBConfigure.resource_path(file_name + '.bak'))
			except:
				self.logger.error(traceback.format_exc())
